...
/**
 * DAO (Data Access Object) para manejar operaciones CRUD sobre la tabla 'entradas'. Cada método
 * interactúa con la base de datos para realizar acciones relacionadas con las entradas del diario
 * de viaje de un usuario.
 */
public class EntradaDAO {
  ...
  public int crear(Entrada entrada) {
    String sql =
        "INSERT INTO entradas (titulo, contenido, fecha_visita, lugar_descripcion, email_usuario) VALUES (?, ?, ?, ?, ?)";
  ...
  /**
   * Obtiene una lista de entradas asociadas a un usuario, ordenadas por fecha de visita
   * descendente.
   *
  ...
  public boolean actualizar(Entrada entrada) {
    String sql =
        "UPDATE entradas SET titulo = ?, contenido = ?, fecha_visita = ?, lugar_descripcion = ? WHERE id = ?";
