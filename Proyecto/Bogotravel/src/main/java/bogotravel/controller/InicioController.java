...
public class InicioController {
  ...
  private final List<String> localidadesBogota =
      Arrays.asList(
          "Usaquén",
          "Chapinero",
          "Santa Fe",
  ...
  private HBox crearVistaLugar(LugarTuristico lugar) {
    ...
    nombre.setStyle(
        "-fx-font-size: 18px; -fx-font-weight: bold;-fx-font-family:Rockwell Condensed;");
  ...
  private void abrirDetalleLugar(LugarTuristico lugar) {
    try {
      FXMLLoader loader =
          new FXMLLoader(getClass().getResource("/bogotravel/view/DetalleLugarView.fxml"));
  ...
  private void verPorVisitar() {
    ...
    List<PorVisitarInfo> lista =
        dao.listarConNombrePorUsuario(SesionActual.getUsuario().getEmail());
    ...
    for (PorVisitarInfo info : lista) {
      ...
      tarjeta.setStyle(
          "-fx-background-color: #FDFEFE; -fx-padding: 15; -fx-background-radius: 10; -fx-border-color: #BFC9CA; -fx-border-radius: 10;");
  ...
  private void abrirFormularioEntrada(ActionEvent event) {
    try {
      FXMLLoader loader =
          new FXMLLoader(getClass().getResource("/bogotravel/view/CrearEntradaView.fxml"));
      Parent root = loader.load();
  ...
  private VBox crearVistaEntrada(Entrada entrada) {
    VBox tarjeta = new VBox(10);
    tarjeta.setStyle(
        "-fx-background-color: #FAFAFA; -fx-padding: 12; -fx-background-radius: 10; -fx-border-color: #cccccc; -fx-border-radius: 10;");
    ...
    Label lugar =
        new Label(
            "Lugar: "
                + Optional.ofNullable(entrada.getLugarDescripcion()).orElse("No especificado"));
